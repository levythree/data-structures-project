...
Tableau* cloneTableau(Tableau* oldTableau) {
    Tableau* newTableau =
        createTableau(oldTableau->variables, oldTableau->equations);
...
Tableau* cloneAndExpand(Tableau* oldTableau) {
    Tableau* newTableau =
        createTableau(oldTableau->variables, oldTableau->equations + 1);
    ...
    for (i = 0; i < oldTableau->rows - 1; i++) {
        for (j = 0; j < oldTableau->columns; j++) {
            if (j == oldTableau->columns - 1) {
                newTableau->matrix[i][newTableau->columns - 1] =
                    oldTableau->matrix[i][j];
            } else {
                newTableau->matrix[i][j] = oldTableau->matrix[i][j];
            }
        }
    ...
    for (j = 0; j < oldTableau->columns; j++) {
        if (j == oldTableau->columns - 1) {
            newTableau->matrix[newObjectiveRow][newTableau->columns - 1] =
                oldTableau->matrix[oldObjectiveRow][j];
        } else {
            newTableau->matrix[newObjectiveRow][j] =
                oldTableau->matrix[oldObjectiveRow][j];
        }
    }
